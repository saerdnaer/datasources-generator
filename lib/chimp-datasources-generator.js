...
program
  ...
  .action(function (directory, location, dataSourceImport) {
    ...
    // TODO: switch to https://github.com/openapitools/openapi-generator
    const swaggerCodegen = path.join(__dirname, "./swagger-codegen-cli.jar");
    const pathToTemplate = path.join(__dirname, "./typescript-fetch");
    // // shelljs.rm("-rf", API_DIR);
    shelljs.mkdir("-p", resolvedPath);
    shelljs.exec(
      `java -jar ${swaggerCodegen} generate -l typescript-fetch --template-dir ${pathToTemplate} -i ${resolvedAPI} -o ${resolvedPath}`
    );
    shelljs.mv(`${resolvedPath}/custom.d.ts`, `${resolvedPath}/types.ts`);
